// ✅ Server Component — auto fetches data if ?id= is present
...
export default async function ResultPage({ searchParams }: { searchParams: { id?: string; email?: string } }) {
  ...
  if (id || email) {
    ...
    const res = await fetch(`${GS_WEB_APP}?${query}`, { cache: "no-store" });
    data = await res.json();
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {!hasData ? (
          <div className="mt-8 space-y-4">
            ...
            <p className="text-sm text-white/50">Tip: You also received a direct link in your email after your submission.</p>
          </div>
        ) : (
